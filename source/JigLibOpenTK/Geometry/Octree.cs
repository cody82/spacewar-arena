...
namespace JigLibX.Geometry
{
    ...
    public class Octree
    {
        ...
        public void BuildOctree( int _maxTrisPerCellNOTUSED, float _minCellSizeNOTUSED)
        {
            ...
            {
                triBoxes[i].Min = Vector3Extensions.Min(positions[tris[i].I0], Vector3Extensions.Min(positions[tris[i].I1], positions[tris[i].I2]));
                triBoxes[i].Max = Vector3Extensions.Max(positions[tris[i].I0], Vector3Extensions.Max(positions[tris[i].I1], positions[tris[i].I2]));

                // get size of the root box
                rootNodeBox.Min = Vector3Extensions.Min(rootNodeBox.Min, triBoxes[i].Min);
                rootNodeBox.Max = Vector3Extensions.Max(rootNodeBox.Max, triBoxes[i].Max);
            }
