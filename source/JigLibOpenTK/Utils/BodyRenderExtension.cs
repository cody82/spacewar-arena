...
{/*
    ...
    public static class BodyRenderExtensions
    {
        ...
        private static List<Vector3> calcCirclePoints(float radius)
        {
            ...
            {
                double stepRad = OpenTKHelper.ToRadians(slice * stepSize);
        ...
        private static void AddShapeToWireframe( List<Vector3> shape, List<VertexPositionColor> wireframe, Matrix4 orientation, Color color )
        {
        ...
        /// <param name="orientation"></param>
        /// <param name="color"></param>
        private static void AddLineToWireframe(Vector3 from, Vector3 to, List<VertexPositionColor> wireframe, Matrix4 orientation, Color color)
        {
            if (wireframe.Count > 0)
            {
                Vector3 v = wireframe[wireframe.Count - 1].Position;
        ...
        /// <param name="orientation"></param>
        /// <param name="color"></param>
        private static void AddLinesToWireframe(List<Vector3> points, List<VertexPositionColor> wireframe, Matrix4 orientation, Color color)
        {
        ...
        public static VertexPositionColor[] GetLocalSkinWireframe(this CollisionSkin skin)
        {
            ...
            for (int i = 0; i < skin.NumPrimitives; i++)
            {
                ...
                Matrix4 trans = p.TransformMatrix;
                ...
                if (p is Sphere)
                {
                    ...
                    AddShapeToWireframe(SpherePoints, wireframe, Matrix4.CreateRotationY(MathHelper.PiOver2) * trans, Color.Red);
                    AddShapeToWireframe(SpherePoints, wireframe, Matrix4.CreateRotationX(MathHelper.PiOver2) * trans, Color.Green);
                ...
                else if (p is Capsule)
                {
                    ...
                    {
                        Vector3 t = Vector3.Transform(v, Matrix4.CreateRotationX(MathHelper.PiOver2));
                        CylPoints.Add(t);
                    }

                    float len = np.Length;

                    SidePoints.Add(Vector3.Transform(new Vector3(np.Radius, len, 0), Matrix4.CreateRotationX(MathHelper.PiOver2)));
                    SidePoints.Add(Vector3.Transform(new Vector3(np.Radius, 0, 0), Matrix4.CreateRotationX(MathHelper.PiOver2)));
                    SidePoints.Add(Vector3.Transform(new Vector3(-np.Radius, 0, 0), Matrix4.CreateRotationX(MathHelper.PiOver2)));
                    SidePoints.Add(Vector3.Transform(new Vector3(-np.Radius, len, 0), Matrix4.CreateRotationX(MathHelper.PiOver2)));

                    // Create Y Rungs
                    AddShapeToWireframe(Ball, wireframe, Matrix4.CreateTranslation(new Vector3(0, 0, 0.0f * len)) * trans, Color.Green);
                    AddShapeToWireframe(Ball, wireframe, Matrix4.CreateTranslation(new Vector3(0, 0, 0.5f * np.Length)) * trans, Color.Green);
                    AddShapeToWireframe(Ball, wireframe, Matrix4.CreateTranslation(new Vector3(0, 0, 1.0f * np.Length)) * trans, Color.Green);

                    // Create Z Profile
                    Matrix4 Zmat = Matrix4.CreateRotationZ(MathHelper.PiOver2);
                    AddShapeToWireframe(CylPoints, wireframe, Matrix4.CreateTranslation(new Vector3(0, 0, np.Length)) * Zmat * trans, Color.Blue);
                    AddShapeToWireframe(CylPoints, wireframe, Matrix4.CreateTranslation(new Vector3(0, 0, 0)) * Zmat * trans, Color.Blue);
                    ...
                    //// Create X Profile 
                    Matrix4 Xmat = Matrix4.Identity;
                    AddShapeToWireframe(CylPoints, wireframe, Matrix4.CreateTranslation(new Vector3(0, 0, np.Length)) * Xmat * trans, Color.Red);
                    AddShapeToWireframe(CylPoints, wireframe, Matrix4.CreateTranslation(new Vector3(0, 0, 0)) * Xmat * trans, Color.Red);
        ...
        public static void TransformWireframe(this Body body, VertexPositionColor[] wireframe)
        {
            ...
            for ( int i = 0; i < wireframe.Length; i++)
            {
                wireframe[i].Position = Vector3.Transform(wireframe[i].Position,
                                            body.Orientation * Matrix4.CreateTranslation(body.Position));
            }
