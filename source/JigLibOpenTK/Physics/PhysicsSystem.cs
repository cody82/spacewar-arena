...
namespace JigLibX.Physics
{
    ...
    public class PhysicsSystem
    {
        ...
        public PhysicsSystem()
        {
            ...
            Gravity = -10.0f * Vector3Extensions.Up;
            ...
            for (int pos = collision.NumCollPts; pos-- != 0; )
            {
                ...
                // first the real impulse
                {
                    ...
                    if (System.Math.Abs(deltaVel) > minVelForProcessing)
                    {
                        ...
                        ptInfo.AccumulatedNormalImpulse = OpenTKHelper.Max(ptInfo.AccumulatedNormalImpulse + normalImpulse, 0.0f);
        ...
        private unsafe bool ProcessCollisionCombined(CollisionInfo collision, float dt, bool firstContact)
        {
                ...
                if (deltaVel > minVelForProcessing)
                {
                    ...
                    if (!body0.Immovable)
                        denominator = body0.InverseMass +
                            Vector3.Dot(N, Vector3.Cross(Vector3Extensions.TransformNormal(Vector3.Cross(R0, N), body0.WorldInvInertia), R0));
                    if ((body1 != null) && (!body1.Immovable))
                        denominator += body1.InverseMass +
                            Vector3.Dot(N, Vector3.Cross(Vector3Extensions.TransformNormal(Vector3.Cross(R1, N), body1.WorldInvInertia), R1));
                    if (denominator < JiggleMath.Epsilon)
            ...
            for (pos = collision.NumCollPts; pos-- != 0; )
            {
                ...
                if (tangentSpeed > minVelForProcessing)
                {
                    ...
                    {
                        denominator = body0.InverseMass +
                            Vector3.Dot(T, Vector3.Cross(Vector3Extensions.TransformNormal(Vector3.Cross(ptInfo.Info.R0, T), body0.WorldInvInertia), ptInfo.Info.R0));
                    }

                    if ((body1 != null) && (!body1.Immovable))
                    {
                        denominator += body1.InverseMass +
                            Vector3.Dot(T, Vector3.Cross(Vector3Extensions.TransformNormal(Vector3.Cross(ptInfo.Info.R1, T), body1.WorldInvInertia), ptInfo.Info.R1));
                    }
        ...
        private void PreProcessCollision(CollisionInfo collision, float dt)
        {
            ...
            for (int pos = 0; pos < collision.NumCollPts; ++pos)
            {
                ...
                else
                {
                    ...
                    approachScale = OpenTKHelper.Clamp(approachScale, JiggleMath.Epsilon, 1.0f);
                    ptInfo.MinSeparationVel = approachScale * (ptInfo.Info.InitialPenetration - allowedPenetration) / OpenTKHelper.Max(dt, JiggleMath.Epsilon);
                }
        ...
        private void PreProcessCollisionFast(CollisionInfo collision, float dt)
        {
            ...
            for (int pos = 0; pos < collision.NumCollPts; ++pos)
            {
                ...
                else
                {
                    ...
                    Vector3Extensions.TransformNormal(ref cross, ref body0.worldInvInertia, out cross);
                ...
                if ((body1 != null) && !body1.Immovable)
                {
                    ...
                    Vector3Extensions.TransformNormal(ref cross, ref body1.worldInvInertia, out cross);
