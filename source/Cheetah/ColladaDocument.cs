...
namespace COLLADA
{
    ...
    public class Document
    {
        ...
        public class Source : Element, ISourceOrVertices
        {
            ...
            public Source(Document doc, string _id, object _array, Accessor _accessor)
                : base(doc, _id)
            {
                accessor = _accessor;
                array = _array;
        ...
        public class Locator
        {
            ...
            private void createLocator(Document doc,string path)
            {
                ...
                else // relative URI or erronous URI
                {
                    if (doc.baseURI != null)
                        ...
                        url = new Uri(path); isRelative = true;
                }
                isInvalid = false;
            ...
            public string Fragment
            {
                get
                {
                    if (!isFragment) throw new Exception("cannot get Fragment of a non Fragment URI" + this.ToString());
        ...
        public class Controller : Element
        {
            ...
            public Controller(Document doc, XmlNode node)
                : base(doc, node)
            {
                if (id == null) throw new Exception("Controller does not have id ! : " + doc.filename);
                ...
                foreach (XmlNode child in node.ChildNodes)
                {
                    switch (child.Name)
                    {
                        case "input":
                            ...
                            break;
                        default:
                            throw new Exception("Invalid node " + child.Name + " type in <sampler>");
                    }
                }
        ...
        public class Animation : Element // id, name
        {
                ...
                : base(doc, node)
            {
                ...
                if (id == null) throw new Exception("Animation does not have id ! : " + doc.filename);
                foreach (XmlNode child in node.ChildNodes)
                {
                    switch (child.Name)
                    {
        ...
        public Document(string name)
            : this(new FileStream(name,FileMode.Open,FileAccess.Read))
        {
            filename = name;
            //if (!File.Exists(filename))
            //    throw new FileNotFoundException("Could ot find file:" + filename);

            int split = baseURI.ToString().LastIndexOf("/");
            //baseURI = new Uri(colladaDocument.BaseURI);
            documentName = baseURI.ToString().Substring(split + 1);
        }

        public Document(Stream stream)
            : this()
        {
            baseURI = new Uri("stream://unknown");
            colladaDocument = new XmlDocument();
            colladaDocument.Load(stream);
            root = colladaDocument.DocumentElement;
            if (root.Name != "COLLADA")
                throw new Exception("This file " + filename + "contains a " + root.Name + ", expected COLLADA");
            ...
            foreach (XmlNode animationLib in animationLibs)
            {
                ...
                {
                    if (animations == null) animations = new List<Animation>();
